...
/*!
 *  @brief        Set the power of the Si24R1        
 *  @param        
              power:
              111	7 dBm
              110	4 dBm
...
{  
    INT8U mask = L01_ReadSingleReg(L01REG_RF_SETUP) & ~0x07; // 清除发射功率控制位  

    switch (power)  
    {  
    case POWER_SI24R1_7DBM:  
        mask |= 0x07; // 设置为 111  
        break;  
    case POWER_SI24R1_4DBM:  
        mask |= 0x06; // 设置为 110  
        break;  
    case POWER_SI24R1_3DBM:  
        mask |= 0x05; // 设置为 101  
        break;  
    case POWER_SI24R1_1DBM:  
        mask |= 0x04; // 设置为 100  
        break;  
    case POWER_SI24R1_0DBM:  
        mask |= 0x03; // 设置为 011  
        break;  
    case POWER_SI24R1_MINUS4DBM:  
        mask |= 0x02; // 设置为 010  
        break;  
    case POWER_SI24R1_MINUS6DBM:  
        mask |= 0x01; // 设置为 001  
        break;  
    case POWER_SI24R1_MINUS12DBM:  
        // 保持为 000  
        break;  
    default:  
        break; // 无效输入  
    }  
    L01_WriteSingleReg(L01REG_RF_SETUP, mask); // 将修改后的值写回寄存器  
}  
/*!
...
void L01_Init(void)
{
    ...
    L01_WriteSingleReg(L01REG_SETUP_RETR,ARD_250US|ARC_10);//ARD:250us,repeat time:10
    L01_SetDataRate(DRATE_2M);
    L01_WriteHoppingPoint(1);
    Si24R1_SetPower(POWER_SI24R1_7DBM);
